...
\todo[inline]{Platforms are current PCs, consoles, and cloud systems as a whole. (We leave out mobile/browser/``classic'' consoles).}
\todo[inline]{Platforms *also* are ecosystems. We need to draw the line somewhere.}
...
generation, i.e., \textit{Nintendo Switch}, \textit{PlayStation 4}, and
\textit{Xbox One}.
While priced lower than \glspl{PC}, console hardware is not componentized
and not upgradeable as easily.
\todo[inline]{PS4 can be used for CG.}

\subsubsection{Streaming-Enabled Devices}
...
$\text{\texteuro} 200$ and is bound to their \gls{cg} service.
Sony (amongst others) have experimented with enabling game streaming
on their TV sets, but removed the option again.

g

\subsection{Gaming Platforms}
\todo[inline]{Vielleicht finden wir da einen treffenderen Subsec-Namen.}
...
Sony's cloud gaming service offers to stream titles from previous
...
Following the discussion of gaming platform characteristics, the
...
The age of a game is computable from its release date. To this end, the

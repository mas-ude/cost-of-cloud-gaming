...
summary(df.hltb)
mean(df.hltb$main_story_length, na.rm = TRUE)
median(df.hltb$main_story_length, na.rm = TRUE)
...
ggplot(df.hltb, aes(x = main_story_length, color=as.factor(platform))) + stat_ecdf() + scale_x_log10()

ggplot(df.hltb, aes(x = main_story_length, color=as.factor(platform))) + stat_ecdf() + scale_x_log10()



p <- ggplot(df.hltb, aes(x = main_story_length)) + stat_density()  + scale_x_log10() #+ xlim(0, 75)#+ scale_x_log10() # + xlim(0, 100)
p <- p + xlab("avg. combined playthrough length (h)") + ylab("density")
p <- p + theme(text = element_text(size=20))
p
ggsave("gamelengths-density.pdf", width=12, height=8)

ggplot(df.hltb, aes(x=main_story_length, fill=as.factor(platform))) + stat_density() + scale_x_log10()
...
df.lengths <- rbind(df.lengths, data.frame(title = df.consolidated.gfnow$name, platform = "Geforce Now", main_story_length = df.consolidated.gfnow$main_story_length))
df.lengths <- rbind(df.lengths, data.frame(title = df.consolidated.psnow$name, platform = "PlayStation Now", main_story_length = df.consolidated.psnow$main_story_length))
df.lengths <- rbind(df.lengths, data.frame(title = df.consolidated.steam$name, platform = "Steam", main_story_length = df.consolidated.steam$main_story_length))
df.lengths <- rbind(df.lengths, data.frame(title = df.hltb$title, platform = "overall", main_story_length = df.hltb$main_story_length))
...
## density plot
p <- ggplot(df.lengths, aes(x = main_story_length, fill = as.factor(platform), color = as.factor(platform))) + geom_density(alpha = 0.4) + scale_x_log10() #+ xlim(0, 75)#+ scale_x_log10() # + xlim(0, 100)
p <- p + xlab("avg. combined playthrough length (h)") + ylab("density")
p <- p + theme(text = element_text(size=20))
p
...
p <- ggplot(df.lengths, aes(x = as.factor(platform), y = main_story_length)) + geom_violin(adjust = .8, scale = "area", draw_quantiles = c(0.25,0.5,0.75), na.rm = TRUE) + scale_y_log10(breaks=nice_breaks, labels=nice_breaks) #+ xlim(0, 75)#+ scale_x_log10() # + xlim(0, 100)
